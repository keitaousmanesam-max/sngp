...
@section('title', 'Maladies et Pathologies')
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h1 class="page-title mb-1"><i class="fas fa-virus me-2"></i>Maladies et Pathologies</h1>
        <p class="text-muted mb-0">Référentiel national des maladies (CIM-10)</p>
    </div>
    <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalCreate">
        <i class="fas fa-plus me-2"></i>Nouvelle Maladie
    </button>
</div>
...
<div class="row g-3 mb-4">
    <div class="col-6 col-xl-3">
        <div class="stats-card total">
            ...
            <div class="stats-label"><i class="fas fa-virus me-1"></i>Total</div>
        </div>
    </div>
    <div class="col-6 col-xl-3">
...
<div style="background: white; border-radius: 12px; padding: 20px 24px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); border: 1px solid #f0f0f0; margin-bottom: 24px;">
    <form method="GET" action="{{ route('maladies.index') }}">
        <div class="row g-3 align-items-end">
            <div class="col-12 col-md-5">
                <label class="form-label fw-semibold small">Rechercher</label>
                <div class="input-group">
                    <span class="input-group-text"><i class="fas fa-search"></i></span>
                    <input type="text" name="search" class="form-control" placeholder="Nom ou code CIM-10..." value="{{ request('search') }}">
                </div>
            </div>
            ...
            <div class="col-6 col-md-4 d-flex gap-2">
                ...
                <a href="{{ route('maladies.index') }}" class="btn btn-outline-secondary"><i class="fas fa-times"></i></a>
            </div>
        </div>
    </form>
</div>
...
<div class="table-card">
    <div style="padding: 20px 24px; border-bottom: 1px solid #F3F4F6;">
        <h6 class="mb-0 fw-semibold" style="color: #374151;">{{ $maladies->total() }} maladie(s)</h6>
    </div>
    <div class="table-responsive">
        <table class="table">
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        <div class="fw-semibold" style="color: #1F2937;">{{ $maladie->nom }}</div>
                        @if($maladie->description)
                        <small class="text-muted">{{ Str::limit($maladie->description, 60) }}</small>
                        @endif
                    </td>
                    <td>
                        @if($maladie->code_cim10)
                        <span style="background: #FEF3C7; color: #92400E; padding: 4px 10px; border-radius: 6px; font-size: 12px; font-family: monospace; font-weight: 700;">
                            {{ $maladie->code_cim10 }}
                        </span>
                        @else
                    ...
                    <td style="color: #6B7280;">{{ $maladie->categorie ?? '—' }}</td>
                    <td>
                        <span class="fw-semibold">{{ $maladie->produits_count }}</span>
                        <small class="text-muted"> médicament(s)</small>
                    </td>
                    <td>
                        @if($maladie->actif)
                    ...
                    <td>
                        <div class="d-flex gap-1">
                            <button class="action-btn edit" title="Modifier"
                                onclick="editMaladie({{ $maladie->id }}, '{{ addslashes($maladie->nom) }}', '{{ $maladie->code_cim10 }}', '{{ addslashes($maladie->description) }}', '{{ $maladie->categorie }}')">
                                <i class="fas fa-edit"></i>
                            </button>
                            <form method="POST" action="{{ route('maladies.toggle', $maladie) }}" class="d-inline">
                                @csrf @method('PATCH')
                                <button class="action-btn toggle" title="{{ $maladie->actif ? 'Désactiver' : 'Activer' }}">
                                    <i class="fas fa-{{ $maladie->actif ? 'pause' : 'play' }}"></i>
                                </button>
                            </form>
                            <form method="POST" action="{{ route('maladies.destroy', $maladie) }}" class="d-inline">
                                @csrf @method('DELETE')
                                <button class="action-btn delete" title="Supprimer"
                                    onclick="return confirm('Supprimer cette maladie ?')">
                ...
                <tr>
                    <td colspan="6">
                        <div style="text-align: center; padding: 60px 20px; color: #9CA3AF;">
                            <i class="fas fa-virus" style="font-size: 48px; opacity: 0.3; display: block; margin-bottom: 16px;"></i>
                            <p class="mb-2 fw-semibold">Aucune maladie trouvée</p>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div style="background: linear-gradient(135deg, #1E3A8A, #3B82F6); padding: 24px; border-radius: 16px 16px 0 0;">
                <h5 style="color: white; margin: 0; font-weight: 700;"><i class="fas fa-plus me-2"></i>Nouvelle Maladie</h5>
            </div>
            <form method="POST" action="{{ route('maladies.store') }}">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="nom" class="form-control" placeholder="Ex: Paludisme" required>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-semibold">Code CIM-10</label>
                            <input type="text" name="code_cim10" class="form-control" placeholder="Ex: B54">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold">Catégorie</label>
                            <input type="text" name="categorie" class="form-control" placeholder="Ex: Infectieuse">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Description</label>
                        <textarea name="description" class="form-control" rows="3" placeholder="Description de la maladie..."></textarea>
                    </div>
                </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div style="background: linear-gradient(135deg, #059669, #10B981); padding: 24px; border-radius: 16px 16px 0 0;">
                <h5 style="color: white; margin: 0; font-weight: 700;"><i class="fas fa-edit me-2"></i>Modifier la Maladie</h5>
            </div>
            <form method="POST" id="formEdit" action="">
                ...
                <div class="modal-body p-4">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="form-label fw-semibold">Code CIM-10</label>
                            <input type="text" name="code_cim10" id="editCode" class="form-control">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold">Catégorie</label>
                            <input type="text" name="categorie" id="editCategorie" class="form-control">
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Description</label>
...
@push('scripts')
<script>
    function editMaladie(id, nom, code, description, categorie) {
        document.getElementById('editNom').value = nom;
        document.getElementById('editCode').value = code || '';
        document.getElementById('editDescription').value = description || '';
        document.getElementById('editCategorie').value = categorie || '';
        document.getElementById('formEdit').action = '/maladies/' + id;
